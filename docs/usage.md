# nf-core/kmerseek: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/kmerseek/usage](https://nf-co.re/kmerseek/usage)
...
```bash
nextflow run nf-core/kmerseek -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/kmerseek
```
...
First, go to the [nf-core/kmerseek releases page](https://github.com/nf-core/kmerseek/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
